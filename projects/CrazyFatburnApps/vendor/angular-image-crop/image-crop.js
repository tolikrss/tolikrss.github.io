...
//======================================================================================================
          ...
          scope.$watch('zoomValue', function(){
			myZoomImage(scope.zoomValue);
		  });

          
          var myMaxZoomedInLevel = 0, myMaxZoomedOutLevel = 2;

          function myTo2Dp(val) {
            return Math.round(val * 1000) / 1000;
          }

          function myZoomImage(val) {

            if (!val) {
              return;
            }
			
            var proposedZoomLevel = myTo2Dp(val);
            if ((proposedZoomLevel < myMaxZoomedInLevel) || (proposedZoomLevel > myMaxZoomedOutLevel)) {
...
//======================================================================================================
		  ...
		  var paddingHorizontal = scope.paddingHorizontal ? Number(scope.paddingHorizontal) : 200,
              paddingVertical = scope.paddingVertical ? Number(scope.paddingVertical) : 200;
          ...
          scope.canvasWidth = scope.width + paddingHorizontal;
          scope.canvasHeight = scope.height + paddingVertical;
        ...
        // default
        //   var minXPos = 0, maxXPos = (paddingHorizontal/2), minYPos = 0, maxYPos = (paddingVertical/2); // for dragging bounds		 
          var minXPos = 0, maxXPos = (paddingHorizontal/2), minYPos = 0, maxYPos = (paddingVertical/2); // for dragging bounds	 
          ...
          // ---------- INLINE STYLES ----------- //
          scope.moduleStyles = {
            width: (scope.width + paddingHorizontal) + 'px',
            height: (scope.height + paddingVertical) + 'px'
          };
          ...
          $img.onload = function() {
            ...
            minLeft = (scope.width + paddingHorizontal) - this.width;
            minTop = (scope.height + paddingVertical) - this.height;
            newWidth = imgWidth;
            newHeight = imgHeight;
            
			if(imgWidth >= imgHeight) {
				maxZoomedInLevel = ($canvas.height - paddingVertical) / imgHeight;
                myMaxZoomedInLevel = ($canvas.height - paddingVertical) / imgHeight;
                scope.minZoomLevel = myMaxZoomedInLevel;
                myMaxZoomedOutLevel = myMaxZoomedInLevel * scope.zoomRatio;
                scope.maxZoomLevel = myMaxZoomedOutLevel;
                scope.zoomValue = (myMaxZoomedOutLevel - myMaxZoomedInLevel) / 2;
			} else {
				maxZoomedInLevel = ($canvas.width - paddingHorizontal) / imgWidth;
                myMaxZoomedInLevel = ($canvas.width - paddingHorizontal) / imgWidth;
                scope.minZoomLevel = myMaxZoomedInLevel;
                myMaxZoomedOutLevel = myMaxZoomedInLevel * scope.zoomRatio;
                scope.maxZoomLevel = myMaxZoomedOutLevel;
                scope.zoomValue = (myMaxZoomedOutLevel - myMaxZoomedInLevel) / 2;
          ...
          function updateDragBounds() {
            ...
            minXPos = $canvas.width - ($img.width * zoom) - (paddingHorizontal/2);
            minYPos = $canvas.height - ($img.height * zoom) - (paddingVertical/2);
          ...
          $finalImg.onload = function() {			
            ...
            tempCanvas.width = this.width - paddingHorizontal;
            tempCanvas.height = this.height - paddingVertical;
            tempCanvas.style.display = 'none';

            var tempCanvasContext = tempCanvas.getContext('2d');
            tempCanvasContext.drawImage($finalImg, -(paddingHorizontal/2), -(paddingVertical/2));
